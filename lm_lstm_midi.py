...
def log_prob_bernoulli(x, mean):
    return x * T.log(mean) + (1 - x) * T.log(1 - mean)
...
def build_rev_model(tparams, options, x, y, x_mask):
    ...
    xr_emb = get_layer('ff')[1](tparams, xr, options, prefix='ff_in_lstm', activ='lrelu')
    ...
    out_mus = get_layer('ff')[1](tparams, out, options, prefix='ff_out_mus_r', activ='lambda x: tensor.nnet.sigmoid(x)')

    # ...
    log_p_y = log_prob_bernoulli(yr, out_mus)
...
def build_gen_model(tparams, options, x, y, x_mask, zmuv, states_rev):
    ...
    out_mus = get_layer('ff')[1](tparams, out, options, prefix='ff_out_mus', activ='lambda x: tensor.nnet.sigmoid(x)')

    # Compute gaussian log prob of target
    log_p_y = log_prob_bernoulli(y, out_mus)
    log_p_y = T.sum(log_p_y, axis=-1)  # Sum over output dim.
    nll_gen = -log_p_y  # NLL
...
def train(dim_input=88,  # input vector dimensionality
    ...
    desc = saveto + 'multi_layer_model_' + str(weight_aux) + '_wgt_aux_' +  str(kl_start) + '_kls_log.txt'
    opts = saveto + 'multi_layer_model_' + str(weight_aux) + '_wgt_aux_' +  str(kl_start) + '_kls_opts.pkl'
    print(desc)
    ...
    file_midi = os.path.join('./experiments/midi/', '%s_raw_batchsize16_seqlen100.npz') % (dataset)
    print("Loading midi from %s" % file_midi)
    data = TimitData(file_midi, batch_size=model_options['batch_size'])
